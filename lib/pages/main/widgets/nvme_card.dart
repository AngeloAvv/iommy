import 'package:flutter/material.dart';
import 'package:flutter_essentials_kit/flutter_essentials_kit.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:iommy/features/localization/extensions/build_context.dart';
import 'package:iommy/features/theme/extensions/device_extension.dart';
import 'package:iommy/models/nvme/nvme.dart';
import 'package:iommy/pages/main/widgets/capability_item.dart';

class NvmeCard extends StatelessWidget {
  final Nvme nvme;
  final String? name;

  const NvmeCard(
    this.nvme, {
    super.key,
    this.name,
  });

  @override
  Widget build(BuildContext context) => Card(
        color: Theme.of(context).colorScheme.surface,
        shape: Theme.of(context).deviceTheme?.nvmeColor?.let(
              (color) => RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(16.0),
                side: BorderSide(
                  color: color,
                  width: 1.0,
                ),
              ),
            ),
        child: Padding(
          padding: const EdgeInsets.all(16.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(
                mainAxisSize: MainAxisSize.min,
                spacing: 8.0,
                children: [
                  FaIcon(
                    FontAwesomeIcons.hardDrive,
                    size: 28.0,
                    color: Theme.of(context).deviceTheme?.nvmeColor,
                  ),
                  Text(
                    name ?? context.l10n?.labelNvme ?? 'labelNvme',
                    style: Theme.of(context).textTheme.titleMedium?.copyWith(
                          color: Theme.of(context).deviceTheme?.nvmeColor,
                        ),
                  ),
                ],
              ),
              CapabilityItem(
                title: nvme.pciId,
                subtitle: context.l10n?.labelIdentifier ?? 'labelIdentifier',
              ),
              CapabilityItem(
                title: nvme.vendorDeviceId,
                subtitle: context.l10n?.labelVendorIdentifier ??
                    'labelVendorIdentifier',
              ),
              CapabilityItem(
                title: nvme.device,
                subtitle: context.l10n?.labelDeviceName ?? 'labelDeviceName',
              ),
              if (nvme.model != null)
                CapabilityItem(
                  title: nvme.model!,
                  subtitle:
                      context.l10n?.labelDeviceModel ?? 'labelDeviceModel',
                ),
              if (nvme.serial != null)
                CapabilityItem(
                  title: nvme.serial!,
                  subtitle: context.l10n?.labelDeviceSerialNumber ??
                      'labelDeviceSerialNumber',
                ),
            ],
          ),
        ),
      );
}
